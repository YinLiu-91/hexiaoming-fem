...
%Xiaoming He, 07/01/2009, 09/24/2011.
...
%basis_type=11:2D Raviart-Thomas FE of first degree.
...
number_of_edges=(N1+1)*N2+N1*(2*N2+1);
...
elseif basis_type==10
   ...
   M=zeros(2,number_of_edges);   
   T=zeros(3,number_of_elements);
   
   for j=1:number_of_edges
       if mod(j,3*N2+1)==0
           M(1,j)=left+(j/(3*N2+1))*h(1)-h(1)/2;
           M(2,j)=top;
       elseif mod(j,3*N2+1)>=1&&mod(j,3*N2+1)<=N2
           M(1,j)=left+fix(j/(3*N2+1))*h(1);
           M(2,j)=bottom+(mod(j,3*N2+1)-1)*h(2)+h(2)/2;          
       else
           M(1,j)=left+fix(j/(3*N2+1))*h(1)+h(1)/2;
           M(2,j)=bottom+(mod(j,3*N2+1)-N2-1)*h(2)/2;                     
       end
   end
   ...
   for n=1:N1*N2
      ...
      T(1,2*n-1)=column*N2+(column-1)*(2*N2+1)+2*row-1;
      T(2,2*n-1)=column*N2+(column-1)*(2*N2+1)+2*row; 
      T(3,2*n-1)=(column-1)*N2+(column-1)*(2*N2+1)+row;

      T(1,2*n)=column*N2+(column-1)*(2*N2+1)+2*row;
      T(2,2*n)=column*N2+column*(2*N2+1)+row;
      T(3,2*n)=column*N2+(column-1)*(2*N2+1)+2*row+1;      
