function r=FE_solution_error_infinity_norm_time_triangle(uh,accurate_function,checked_time,left,right,bottom,top,h_partition,basis_type,derivative_degree_x,derivative_degree_y,Gauss_point_number)
...
for n=1:number_of_elements
    ...
    temp=max(abs(feval(accurate_function,checked_time,Gauss_point_local_triangle(:,1),Gauss_point_local_triangle(:,2))-FE_solution_triangle(Gauss_point_local_triangle(:,1),Gauss_point_local_triangle(:,2),uh_local,vertices,basis_type,derivative_degree_x,derivative_degree_y)));    
    if temp>r
        r=temp;
    end
end