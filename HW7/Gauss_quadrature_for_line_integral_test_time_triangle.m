function r=Gauss_quadrature_for_line_integral_test_time_triangle(coefficient_function_name,current_time,Gauss_coefficient_reference_1D,Gauss_point_reference_1D,end_point_1,end_point_2,test_vertices,test_basis_type,test_basis_index,test_derivative_degree_x,test_derivative_degree_y)
%Xiaoming He, 07/17/2009.
%Use Gauss quadrature to compute a line integral on an edge for a vector depending on time.
...
%current_time: the time when we want to evaluate the coefficient function.
...
%The edge is horizontal.
    ...
    for i=1:Gpn
         r=r+Gauss_coefficient_local_1D(i)*feval(coefficient_function_name,current_time,Gauss_point_local_1D(i),end_point_1(2))*triangular_local_basis(Gauss_point_local_1D(i),end_point_1(2),test_vertices,test_basis_type,test_basis_index,test_derivative_degree_x,test_derivative_degree_y);
    end    
...
%The edge is vertical.
    ...
    [Gauss_coefficient_local_1D,Gauss_point_local_1D]=generate_Gauss_local_1D(Gauss_coefficient_reference_1D,Gauss_point_reference_1D,lower_bound,upper_bound);
    for i=1:Gpn
         r=r+Gauss_coefficient_local_1D(i)*feval(coefficient_function_name,current_time,end_point_1(1),Gauss_point_local_1D(i))*triangular_local_basis(end_point_1(1),Gauss_point_local_1D(i),test_vertices,test_basis_type,test_basis_index,test_derivative_degree_x,test_derivative_degree_y);
    end 
...
%The slope of the edge is in (0,infinity).
    ...
    for i=1:Gpn
         ...
         r=r+Gauss_coefficient_local_1D(i)*Jacobi*feval(coefficient_function_name,current_time,x,y)*triangular_local_basis(x,y,test_vertices,test_basis_type,test_basis_index,test_derivative_degree_x,test_derivative_degree_y);
    end 
