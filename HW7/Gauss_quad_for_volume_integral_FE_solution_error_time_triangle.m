function r=Gauss_quad_for_volume_integral_FE_solution_error_time_triangle(uh_local,accurate_function,checked_time,vertices,Gauss_coefficient_reference_triangle,Gauss_point_reference_triangle,basis_type,derivative_degree_x,derivative_degree_y)
...
%checked_time: the time when we want to check the error of numerical solution.
...
for i=1:Gpn
    r=r+Gauss_coefficient_local_triangle(i)*(feval(accurate_function,checked_time,Gauss_point_local_triangle(i,1),Gauss_point_local_triangle(i,2))-FE_solution_triangle(Gauss_point_local_triangle(i,1),Gauss_point_local_triangle(i,2),uh_local,vertices,basis_type,derivative_degree_x,derivative_degree_y))^2;
end